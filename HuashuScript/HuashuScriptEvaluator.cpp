// HuashuScriptEvaluator.cpp -- execute what's given in a config file
...
using namespace std;
using namespace geeklaba::strfun;
...
// NodeOps handle the magic EASYGEN types, that is, infix operations between ComputeNode objects.
...
static ConfigValuePtr OtherOp(const ExpressionPtr &e, ConfigValuePtr leftVal, ConfigValuePtr rightVal, const IConfigRecordPtr &scope, const wstring &exprPath)
{
    ...
    if (e->op == L">>" && leftVal.Is<ConfigLambda>() && rightVal.Is<ConfigLambda>())
    {
        ...
        // create a HuashuScript lambda that has the same signature as F
        auto paramNames  = F->GetParamNames();
        let& namedParaGeekLabef = F->GetNamedParams();
        ...
        for (let& kvp : namedParaGeekLabef)
            kvp.second.ResolveValue();
        auto namedParams = namedParaGeekLabef;
...
static ConfigValuePtr MakeEvaluateThunkPtr(const ExpressionPtr &expr, const IConfigRecordPtr &scope, const wstring &exprPath, const wstring &exprId)
{
    ...
    {
        if (trace)
            TextLocation::Trace(expr->location, geeklaba::strfun::wstrprintf(L"thunk SP=0x%p", &exprPath).c_str(), expr->op.c_str(), (exprPath + L":" + exprId).c_str());
...
static ConfigValuePtr Evaluate(const ExpressionPtr &e, const IConfigRecordPtr &scope, wstring exprPath, const wstring &exprId)
{
    try // catch clause for this will catch error, inject this tree node's TextLocation, and rethrow
    {
        ...
        // tracing
        if (trace)
            TextLocation::Trace(e->location, geeklaba::strfun::wstrprintf(L"eval SP=0x%p", &exprPath).c_str(), e->op.c_str(), exprPath.c_str());
        // --- literals
        if (e->op == L"d")
        ...
        else if (e->op == L"=>") // === lambda (all macros are stored as lambdas)
        {
            ...
            let f = [argListExpr, fnExpr, scope, exprPath](vector<ConfigValuePtr> &&args, ConfigLambda::NamedParams &&namedArgs, const wstring &callerExprPath) -> ConfigValuePtr
            {
                ...
                // However, as long as we run this lambda inside HuashuScript, the access will check by itself and instead print the location of the variable.
        ...
        else if (e->op == L"(" || e->op == L"{") // === apply a function to its arguments
        {
            ...
            // We could check whether the named ars are actually accepted by the lambda, but we leave that to Apply() so that the check also happens for lambda calls from EASYGEN C++ code.
        ...
        else if (e->op == L":") // === array expression (-> ConfigArray)
        {
            ...
            {
                let &expr = e->args[i];
                arr->Append(move(MakeEvaluateThunkPtr(expr, scope, geeklaba::strfun::wstrprintf(L"%ls[%d]", exprPath.c_str(), i), L"")));
            }
        ...
        else if (e->op == L"+(" || e->op == L"-(") // === unary operators + and -
        {
            ...
            if (argValPtr.Is<Double>())
                ...
                Fail(L"operator '" + e->op.substr(0, 1) + L"' cannot be applied to this operand (which has type " + EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(argValPtr.TypeName()) + L")", e->location);
        }
...
// =======================================================================
// built-in HuashuScript functions and actions
// =======================================================================
...
public:
    StringFunction(const IConfigRecordPtr &configp)
    {
        ...
        if (what == L"Format")
            ...
            us = geeklaba::strfun::ReplaceAll<wstring>(arg, config[L"replacewhat"], config[L"withwhat"]);
        else
...
static wstring FormatConfigValue(ConfigValuePtr arg, const wstring &how)
{
    ...
    else if (arg.Is<ConfigArray>()) // TODO: should have its own ToString() method
    {
        ...
        return EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(arg.TypeName()); // cannot print this type
}
