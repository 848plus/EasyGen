...
using namespace std;
using namespace geeklab1::strfun;
...
private:
    ...
    Token NextToken()
    {
        ...
        else if (iswdigit(ch) || (ch == L'.' && iswdigit(GotCharPtr()[1]))) // --- number
        {
            ...
            t.number = wcstod(beginPtr, &endPtr); // BUGBUG: this seems to honor locale settings. We need one that doesn't. With this, EASYGEN won't parse right in Germany.
            if (endPtr == beginPtr)
...
public:
    ...
    ExpressionPtr ParseExpression(int requiredPrecedence, bool stopAtNewline)
    {
        ...
        for (;;)
        {
            ...
            // while continuing to parse across newlines when syntactically meaningful (there is no ambiguity in HuashuScript).
