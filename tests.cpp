...
void TestBing(const ConfigParameters& config)
{
    ...
    wstring modelPath = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(outDir).append(L"\\bingranknet.dnn");
...
void TestConfiguration(const ConfigParameters& configBase)
{
    ...
    for (auto iterMacro = configMacros.begin(); iterMacro != configMacros.end(); iterMacro++)
    {
             ...
             iter != configNodes.end(); iter++)
        {
            ...
            nodeName = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(iter->first);
        ...
        {
            std::wstring nodeName = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(iter->first);
            ConfigArray configNode = iter->second;
