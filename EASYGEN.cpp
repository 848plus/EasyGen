...
//
// EASYGEN.cpp : Defines the entry point for the console application.
//
...
#include "HuashuScriptEvaluator.h"
#include "HuashuScriptParser.h"
#include "PerformanceProfiler.h"
#include "EASYGENLibrary.h"
...
using namespace std;
using namespace EasyCheng::GEEKLAB;
using namespace EasyCheng::GEEKLAB::EASYGEN;
...
void CheckSupportForGpu(DEVICEID_TYPE deviceId)
{
    ...
    {
        InvalidArgument("EASYGEN: The GPU (%s) has compute capability %d.%d.  EASYGEN is only supported on GPUs with compute capability 3.0 or greater", 
                        gpuData.name.c_str(), gpuData.versionMajor, gpuData.versionMinor);
    }
    else if (gpuData.validity == GpuValidity::UnknownDevice)
    {
        InvalidArgument("EASYGEN: Unknown GPU with Device ID %d.", gpuData.deviceId);
    }
}
#endif
...
void DoCommands(const ConfigParameters& config, const shared_ptr<MPIWrapper>& mpi)
{
    ...
    for (int i = 0; i < command.size(); i++)
    {
        ...
        for (int j = 0; j < action.size(); j++)
        {
            if (action[j] == "train" || action[j] == "trainRNN")
            {
                ...
                if (progressTracing)
                {
                    LOGPRINTF(stderr, "EASYGENModelPath: %ls\n", modelPath.c_str());
                    LOGPRINTF(stderr, "EASYGENCommandTrainInfo: %s : %d\n", command[i].c_str(), (int)maxEpochs);
                }
    ...
    if (progressTracing)
    {
        LOGPRINTF(stderr, "EASYGENCommandTrainInfo: EASYGENNoMoreCommands_Total : %d\n", (int)fullTotalMaxEpochs);
    }
        ...
        for (int j = 0; j < action.size(); j++)
        {
            ...
            if ((mpi == nullptr) || (commandstoRunOnAllRanks.find(thisAction) != commandstoRunOnAllRanks.end()) || mpi->IsMainNode())
            {
                if (thisAction == "train" || thisAction == "trainRNN")
                {
                    if (progressTracing)
                    {
                        LOGPRINTF(stderr, "EASYGENCommandTrainBegin: %s\n", command[i].c_str());
                    }
                }
                ...
                if (thisAction == "train" || thisAction == "trainRNN")
                {
                    if (progressTracing)
                    {
                        LOGPRINTF(stderr, "EASYGENCommandTrainEnd: %s\n", command[i].c_str());
                    }
...
{
    LOGPRINTF(stderr, "-------------------------------------------------------------------\n");
    LOGPRINTF(stderr, "Usage: easygen configFile=yourConfigFile\n");
    LOGPRINTF(stderr, "For detailed information please consult the EASYGEN book\n");
    LOGPRINTF(stderr, "\"An Introduction to Computational Networks and the Computational Network Toolkit\"\n");
    LOGPRINTF(stderr, "-------------------------------------------------------------------\n");
}

// ---------------------------------------------------------------------------
// main() for use with HuashuScript as entire config language (this is experimental)
// ---------------------------------------------------------------------------
...
int wmainWithBS(int argc, wchar_t* argv[]) // called from wmain which is a wrapper that catches & reports Win32 exceptions
{
    ...
    wstring startupMessage = geeklaba::strfun::wstrprintf(L"running on %ls at %ls\n", EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(GetHostName()).c_str(), EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(TimeDateStamp()).c_str());
    startupMessage += geeklaba::strfun::wstrprintf(L"command line: %ls", exePath.c_str());
    ...
    while (!args.empty())
    {
        ...
        if (option == L"-f" || option == L"--file") // -f defines source files
            Append(sourceFiles, geeklaba::strfun::split(ConsumeArg(args), L";"));
        else if (option == L"-I") // -I declares an include search path
            Append(includePaths, geeklaba::strfun::split(ConsumeArg(args), L";"));
    ...
    // compile the HuashuScript
    wstring bs = L"[\n";
    bs += L"include \'easygen.core.bs'"; // start with including the standard macros

    // Note: Using lowercase ^^ here to match the Linux name of the EASYGEN exe.
    ...
    // parallel training
    shared_ptr<EasyCheng::GEEKLAB::EASYGEN::MPIWrapper> mpi;
    ...
    if (logpath != L"")
    {
        if (paralleltrain && mpi->CurrentNodeRank() != 0)
            logpath += geeklaba::strfun::wstrprintf(L".rank%d", (int) mpi->CurrentNodeRank());

        RedirectStdErr(logpath, config(L"appendLogFile", false));
        LOGPRINTF(stderr, "%ls\n", startupMessage.c_str());
        ::EASYGEN::Internal::PrintBuiltInfo();
    }

    // echo gpu info to log
#ifndef CPUONLY
    ::EASYGEN::Internal::PrintGpuInfo(GetAllGpusData());
#endif
...
// ---------------------------------------------------------------------------
// main() for EASYGEN config language (this is the current way of using EASYGEN)
// ---------------------------------------------------------------------------

static void PrintBanner(int argc, wchar_t* argv[], const string& timestamp)
{
#ifndef EASYGEN_VERSION_BANNER
#error EASYGEN_VERSION_BANNER must be set
#endif
#define MACRO_TO_STRING(s) #s
    fprintf(stderr, "EASYGEN %s (", MACRO_TO_STRING(EASYGEN_VERSION_BANNER));
#ifdef _GIT_EXIST
...
int wmainOldEASYGENConfig(int argc, wchar_t* argv[])
{
    ...
    if (logpath != L"")
    {
        ...
        if (paralleltrain && mpi->CurrentNodeRank() != 0)
            logpath += geeklaba::strfun::wstrprintf(L".rank%d", mpi->CurrentNodeRank());
        RedirectStdErr(logpath, config(L"appendLogFile", false));
        if (traceLevel == 0)
    ...
    if (config.Exists("type"))
        InvalidArgument("EASYGEN: Use of 'type' parameter is deprecated, it is called 'precision' now.");
    ...
    if (type == "float16")
        ...
        RuntimeError("EASYGEN: Invalid precision string: \"%s\", must be \"float16\" or \"float\" or \"double\"", type.c_str());
...
{
    EasyCheng::GEEKLAB::EASYGEN::DebugUtil::PrintCallStack();
...
int wmain1(int argc, wchar_t* argv[]) // called from wmain which is a wrapper that catches & reports Win32 exceptions
{
    ...
    {        
        if (argc <= 1)
        {
            ::EASYGEN::Internal::PrintBuiltInfo(); // print build info directly in case that user provides zero argument (convenient for checking build type)
        ...
        // detect legacy EASYGEN configuration
        bool isOldEASYGENConfig = false;
        for (int i = 0; i < argc && !isOldEASYGENConfig; i++)
            isOldEASYGENConfig |= !_wcsnicmp(L"configFile=", argv[i], 11);

        if (isOldEASYGENConfig)
            return wmainOldEASYGENConfig(argc, argv);

        // run from HuashuScript
...
static void LogDelayLoadError(PEXCEPTION_POINTERS pExcPointers)
{
    if (pExcPointers->ExceptionRecord->ExceptionCode == EXCEPTION_DLL_NOT_FOUND)
    {
        ...
        LOGPRINTF(stderr, "EASYGEN: Failed to load DLL '%s'.\n", pDelayLoadInfo.szDll);
    }
}
...
int wmain(int argc, wchar_t* argv[]) // wmain wrapper that reports Win32 exceptions
{
    ...
    __try
    {
        ...
        if (_CrtSetReportHook2(_CRT_RPTHOOK_INSTALL, HandleDebugAssert) == -1) {
            LOGPRINTF(stderr, "EASYGEN: _CrtSetReportHook2 failed.\n");
            return -1;
        }
    ...
    __except (LogDelayLoadError(GetExceptionInformation()), EXCEPTION_EXECUTE_HANDLER)
    {
        ...
        LOGPRINTF(stderr, "EASYGEN: Caught Win32 exception 0x%08x%s.\n", (unsigned int)code, msg);
        fflush(stderr);
        exit(EXIT_FAILURE);
    }
}
