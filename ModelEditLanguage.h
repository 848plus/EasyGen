...
namespace EasyCheng { namespace GEEKLAB { namespace EASYGEN {
...
public:
    ...
    vector<ComputationNodeBasePtr> FindSymbols(const std::string& symbol, NetNdl<ElemType>*& netNdl)
    {
        ...
        wstring name = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(search);
    ...
    vector<GenNameValue> GenerateNames(const std::string& symbolIn, const std::string& symbolOut, NetNdl<ElemType>*& netNdlIn, NetNdl<ElemType>*& netNdlOut)
    {
        ...
        wstring nameOut = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(search);
        ...
        std::wstring first = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(symbolOut.substr(firstStartOut, firstCountOut));
        std::wstring second = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(symbolOut.substr(secondStartOut, secondCountOut));
    ...
    void CopySubTree(const std::string& symbolFrom, const std::string& toCNName, const std::string toNamePrefix, CopyNodeFlags copyFlags)
    {
        ...
        std::wstring toNamePrefixW = EasyCheng::GEEKLAB::EASYGEN::ToFixedWStringFromMultiByte(toNamePrefix);
    ...
    {
        wstring modelFormat = L"easygen"; // default
        for (size_t paramNumber = params.size(); paramNumber > numFixedParams; paramNumber--)
        {
            // process optional parameter if it exists
            std::string propName, value;
            if (OptionalParameter(params[paramNumber - 1], propName, value))
            {
                ...
                {
                    if (EqualInsensitive(value, "easygen"))
                    {
                        modelFormat = L"easygen";
                    }
                    else if (EqualInsensitive(value, "easygen_legacy_no_tensorlib")) // model of late 2015 which had a bug in setting InputValue's tensor dimensions
                    {
                        modelFormat = L"easygen_legacy_no_tensorlib";
                    }
                    else
                    {
                        RuntimeError("Invalid optional parameter value %s, valid values are: format=(easygen)", value.c_str());
                    }
                }
                else
                {
                    RuntimeError("Invalid optional parameter %s, valid optional parameters: format=(easygen)", propName.c_str());
                }
            }
        }
